...
void loop() {
  ...
  // Comprobar si hay datos en el puerto serie
  if (Serial.available() > 0) {
    String message = Serial.readStringUntil('\n'); // Leer mensaje completo hasta el salto de línea
    sendTextToLCD(message); // Enviar mensaje al LCD
  }
...
// Función para enviar un mensaje de texto al LCD en la dirección /lcd
void sendTextToLCD(String message) {
  String mensaje = "/lcd " + message; // Construir mensaje con el texto recibido

  // Enviar el mensaje OSC a la dirección /lcd
  Udp.beginPacket(serverIP, remotePort);
  Udp.print(mensaje);
  Udp.endPacket();
}
