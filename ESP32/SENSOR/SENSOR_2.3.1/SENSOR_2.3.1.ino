...
// === Paquete persistente listo para enviar ===
...
void onI2CRequest() {
    if (packetReady) {
        ...
        // Enviar paquete vacío si aún no está listo
        uint8_t empty = 0;
        Wire.write(&empty, 1);
    }
}
...
void loop() {
    ...
    if (now - lastUpdate >= REQUEST_INTERVAL_MS) {
        ...
        // Precalcular el paquete de datos para el maestro
        sensorPacket = sensors.packetize();
        packetReady = true;
