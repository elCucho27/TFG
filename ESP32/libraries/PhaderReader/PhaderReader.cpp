#include "PhaderReader.h"
#include <Arduino.h>

PhaderReader::PhaderReader(uint8_t pin)
: _pin(pin),
  _filteredVal(0),
  _rawVal(0) {
}

void PhaderReader::begin() {
...
uint16_t PhaderReader::getRawValue() const {
    return _rawVal;                           // Devuelve el último valor directamente leído del ADC
}

uint16_t PhaderReader::getValue() const {
