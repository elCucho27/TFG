#include <WiFi.h>
#include <WiFiUdp.h>
#include <MIDIUSB.h>  // Incluimos la librería MIDIUSB
...
void setup() {
  ...
  Serial.println("Esperando mensajes OSC...");
}
...
void loop() {
  ...
  // Enviar mensajes MIDI a través de USB
  MidiUSB.flush();  // Enviar cualquier mensaje MIDI pendiente
}
...
void enviarMIDI(int control, int valor) {
  midiEventPacket_t event = {0x0B, 0xB0, control, valor};  // Canal 1 (0xB0), Control Change
  MidiUSB.sendMIDI(event);  // Enviar el mensaje MIDI
